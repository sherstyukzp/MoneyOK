...
struct TransactionCallView: View {
    ...
    @ObservedObject var transactionListItem: TransactionEntity
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            Text("Sum : \(transactionListItem.sumTransaction)").font(.title2)
                .foregroundColor(Color(transactionListItem.sumTransaction < 0 ? .red : .blue))
        }
    }
}
...
struct TransactionCallView_Previews: PreviewProvider {
    static var previews: some View {
        TransactionCallView(transactionListItem: TransactionEntity(), account: AccountEntity())
    }
}
